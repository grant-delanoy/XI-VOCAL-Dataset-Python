...
def inter_observer_variability(data, tee):
    ...
    # Bland-Altman plots with detailed analysis
    ...
    for system in SYSTEMS:
        for slices in SLICE_NUMBERS:
            ...
            plt.scatter(mean_vol, diff_vol, color="blue", alpha=0.5)
            plt.axhline(mean_diff, color="red", linestyle="--", label=f"Mean Diff = {mean_diff:.2f}")
            plt.axhline(loa_upper, color="green", linestyle="--", label=f"±1.96 SD = [{loa_lower:.2f}, {loa_upper:.2f}]")
            plt.axhline(loa_lower, color="green", linestyle="--")
            plt.xlabel("Mean Volume (mL)")
            plt.ylabel("Difference (Observer 1 - Observer 2) (mL)")
            plt.title(f"Bland-Altman Plot: {system}, Slices {slices}")
            plt.legend()
            plt.savefig(os.path.join(OUTPUT_DIR, f"bland_altman_{system}_{slices}.png"))
            plt.close()
            print(f"Saved Bland-Altman plot for {system}, Slices {slices}", file=tee)
...
def bias_assessment(data, tee):
    ...
    sns.boxplot(x="Slices", y="Difference", hue="System", data=data)
    plt.axhline(0, color="red", linestyle="--", label="No Bias (Difference = 0)")
    plt.xlabel("Number of Slices")
    plt.ylabel("Difference from True Volume (mL)")
    plt.title("Difference from True Volume by System and Slices")
    plt.legend()
    plt.savefig(os.path.join(OUTPUT_DIR, "boxplot_diff_system_slices.png"))
    plt.close()
    print("Saved boxplot: boxplot_diff_system_slices.png", file=tee)
    
    plt.figure(figsize=(10, 6))
    sns.boxplot(x="Slices", y="Difference", hue="Observer", data=data)
    plt.axhline(0, color="red", linestyle="--", label="No Bias (Difference = 0)")
    plt.xlabel("Number of Slices")
    plt.ylabel("Difference from True Volume (mL)")
    plt.title("Difference from True Volume by Observer and Slices")
    plt.legend()
    plt.savefig(os.path.join(OUTPUT_DIR, "boxplot_diff_observer_slices.png"))
    plt.close()
    print("Saved boxplot: boxplot_diff_observer_slices.png", file=tee)
